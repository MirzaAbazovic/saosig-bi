ALTER TABLE `dw`.`fact_claim` 
ADD COLUMN `dim_loss_time_id` INT NULL AFTER `paid_amount`;


DELIMITER $$
CREATE DEFINER=`administrator`@`%` PROCEDURE `paid_claims_to_dw`(IN dateFrom Datetime,IN dateTo Datetime)
BEGIN
...
SELECT mc.sql_id,mc.dw_id,fc.fact_claim_id , izv.eventdate,DATE_FORMAT(izv.eventdate,'%Y%m%d') as dim_loss_time_id
FROM dw.fact_claim fc join insurance_staging.map_claim mc
ON mc.dw_id=fc.dim_claim_id
JOIN insurance_staging.izv_analitika_po_isplatama izv on izv.claim_id=mc.sql_id
order by izv.eventDate; 

UPDATE dw.fact_claim fc
JOIN insurance_staging.map_claim mc
ON mc.dw_id=fc.dim_claim_id
JOIN insurance_staging.izv_analitika_po_isplatama izv on izv.claim_id=mc.sql_id
SET fc.dim_loss_time_id = DATE_FORMAT(izv.eventdate,'%Y%m%d');

CALL `dw`.`dim_time_insert`('2000-01-01', '2011-12-31');


SELECT distinct izv.eventdate
FROM dw.fact_claim fc join insurance_staging.map_claim mc
ON mc.dw_id=fc.dim_claim_id
JOIN insurance_staging.izv_analitika_po_isplatama izv on izv.claim_id=mc.sql_id
order by izv.eventDate ;
